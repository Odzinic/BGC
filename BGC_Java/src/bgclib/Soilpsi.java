...
public class Soilpsi {
    ...
    public boolean soilpsi(final Siteconst sitec, double soilw, double psi, double vwc_out) {
		 ...
		 * (kg/m2) water mass per unit area sitec.soil_depth (m) effective soil
		 * depth sitec.soil_b (DIM) slope of log(psi) vs log(rwc) sitec.vwc_sat
		 * (DIM) volumetric water content at saturation sitec.psi_sat (MPa) soil
		 * matric potential at saturation output: psi_s (MPa) soil matric
		 * potential
		 * 
		 * uses the relation: psi_s = psi_sat * (vwc/vwc_sat)^b
		 * 
		 * For further discussion see: Cosby, B.J., G.M. Hornberger, R.B. Clapp,
		 * and T.R. Ginn, 1984. A statistical exploration of the relationships
		 * of soil moisture characteristics to the physical properties of soils.
		 * Water Res. Res. 20:682-690.
		 * 
