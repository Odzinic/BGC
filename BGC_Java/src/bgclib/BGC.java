...
public class BGC {
	...
	public int bgc(BGCIn bgcin, BGCOut bgcout, int mode){
		...
		Annual_Rates annrate;
		Atm_Pres atmpres;
		Baresoil_Evap barevap;
		Canopy_Et canet;
		Check_Balance checbal;
		Make_Zero_Flux_Struct mzf;
		Restart_IO restio;
		FirstDay firday;
		Zero_Srcsnk zersrc;
		Get_NDep getndep;
		Get_CO2 getco2;
		Precision_Control prectrl;
		Photosynthesis photsynth;
		DayMet dmet;
		DayPhen dphen;
		Radtrans ratran;
		Prcp_Route prcout;
		...
		Summary_Fun sumfun;
		Phenology_Fun phenfun;
		Output_Map_Init outmapin;
		Output_ASCII outasc;
		Prephenology prephen;
		
		
		String BV_DIAG = String.valueOf(Constant.BV_DIAG.getValue());
		String BV_ERROR = String.valueOf(Constant.BV_ERROR.getValue());
		String BV_DETAIL = String.valueOf(Constant.BV_DETAIL.getValue());
		...
		double[] dayarr, monavgarr, annavgarr, annarr;
		...
		System.out.printf(BV_DIAG, "done copy input\n");
...
//		System.out.printf(BV_DIAG, "done allocate out arrays\n");
		...
		if (ok == 1 && outmapin.output_map_init(output_map,metv,ws,wf,cs,cf,ns,nf,phen,
		...
		if (ok == 1 && mzf.make_zero_flux_struct(zero_wf, zero_cf, zero_nf) == 1)
		{
		...
		if (ok == 1 && atmpres.atm_pres(sitec.elev, metv.pa) == 1)
		{
		...
		if (ok == 1 && prephen.prephenology(ctrl, epc, sitec, metarr, phenarr) == 1)
		{
		...
		if (ok == 1 && ctrl.read_restart == 1)
		{
			if (ok == 1 && restio.restart_input(ctrl, ws, cs, ns, epv, metyr,
		...
		/* no restart file, user supplies initial conditions */
		{
			...
			the first metday */
			if (ok == 1 && firday.firstday(epc, cinit, epv, phenarr, cs, ns) == 1)
			{
			...
			System.out.printf(BV_DIAG, "done firstday\n");
		}

		/* zero water, carbon, and nitrogen source and sink variables */
		if (ok == 1 && zersrc.zero_srcsnk(cs,ns,ws,summary) == 1)
		{
		...
		for (simyr=0 ; ok == 1 && simyr<tmpyears ; simyr++)
		{
			if (mode == MODE_MODEL)
			{
			...
			if (metyr == ctrl.metyears)
			{
				if (mode == MODE_MODEL)
				{
					if (ctrl.onscreen == 1) System.out.printf(BV_DETAIL, "Resetting met data for cyclic input\n");
				}
				if (mode == MODE_SPINUP)
				{
					System.out.printf(BV_DIAG, "Resetting met data for cyclic input\n");
				}
				metyr = 0;
			}
			...
			if (mode == MODE_MODEL)
			{
				...
				if (ctrl.onscreen == 1) System.out.printf(BV_DETAIL, "Year: %6d\n",ctrl.simstartyear+simyr);
			}
			else if (mode == MODE_SPINUP)
			{
				/* output to screen to indicate start of simulation year */
			...
			if (mode == MODE_MODEL)
			{
				...
				else 
				{
	                ...
	                System.out.printf(BV_DIAG,"CO2 val: %lf Year: %i\n",metv.co2,(ctrl.simstartyear+simyr));
					if(metv.co2 < -999)
					{
						System.out.printf(BV_ERROR,"Error finding CO2 value for year: %i\n",(ctrl.simstartyear+simyr));
						break;
					}
				...
				if(bgcin.ndepctrl.varndep == 1 && mode == MODE_MODEL)
				{
					...
					if(daily_ndep < -999)
					{
						System.out.printf(BV_ERROR, "Error finding NDEP for year: %i\n",(ctrl.simstartyear+simyr));
						break;
					}
					else
					{
						System.out.printf(BV_DIAG, "Using annual NDEP value: %lf\n",daily_ndep);				
						daily_ndep /= 365.0;	
					}
				}
			}
			...
			for (yday=0 ; ok == 1 && yday<365 ; yday++)
			{
				...
				if (ok == 1 && dmet.daymet(metarr, metv, metday) == 1)
				{
				...
				if (ok == 1 && dphen.dayphen(phenarr, phen, metday) == 1)
				{
				...
				/* phenology fluxes */
				if (ok == 1 && phenfun.phenology(epc, phen, epv, cs, cf, ns, nf) == 1)
				{
				...
				if (ok == 1 && ratran.radtrans(cs, epc, metv, epv, sitec.sw_alb) == 1)
				{
				...
				if (ok == 1 && metv.prcp == 1 && prcout.prcp_route(metv, epc.int_coef, epv.all_lai,
					wf) == 1)
				{
				...
				if (ok == 1 && ws.snoww == 1 && snmelt.snowmelt(metv, wf, ws.snoww) == 1)
				{
				...
				if (ok == 1 && ws.snoww == 0 && barevap.baresoil_evap(metv, wf, epv.dsr) == 1)
				{
				...
				if (ok == 1 && sopsi.soilpsi(sitec, ws.soilw, epv.psi, epv.vwc) == 1)
				{
				...
				if (ok == 1 && maresp.maint_resp(cs, ns, epc, metv, cf, epv) == 1)
				{
				...
				if (ok == 1 && cs.leafc == 1 && metv.dayl == 1)
				{
					/* conductance and evapo-transpiration */
					if (ok == 1 && canet.canopy_et(metv, epc, epv, wf, 1) == 1)
					{
				...
				if (ok == 1 && cs.leafc == 1 && phen.remdays_curgrowth == 1 && metv.dayl == 1)
				{
					if (ok == 1 && photsynth.total_photosynthesis(metv, epc, epv, cf, psn_sun, psn_shade) == 1) 
					{
				...
				/* calculate outflow */
				if (ok == 1 && oflow.outflow(sitec, ws, wf) == 1)
				{
				...
				if (ok == 1 && dcomp.decomp(metv.tsoil,epc,epv,sitec,cs,cf,ns,nf,nt) == 1)
				{
				...
				if (mode == MODE_MODEL)
				{
					if (ok == 1 && dalloc.daily_allocation(cf,cs,nf,ns,epc,epv,nt,1.0,(int) MODE_MODEL) == 1)
					{
				...
				else if (mode == MODE_SPINUP)
				{
					...
					{
						if (ok == 1 && dalloc.daily_allocation(cf,cs,nf,ns,epc,epv,nt,naddfrac,(int) MODE_SPINUP) == 1)
						{
							System.out.printf(BV_ERROR, "Error in daily_allocation() from bgc.c\n");
							ok=0;
						}
				...
				System.out.printf(BV_DIAG, "%d\t%d\tdone daily_allocation\n",simyr,yday);
				...
				if (ok == 1 && annual_alloc == 1)
				{
					if (ok == 1 && annrate.annual_rates(epc,epv) == 1)
					{
				...
				if (ok == 1 && groresp.growth_resp(epc, cf) == 1)
				{
				...
				if (ok == 1 && statup.daily_water_state_update(wf, ws) == 1)
				{
				...
				if (ok == 1 && statup.daily_carbon_state_update(cf, cs, annual_alloc,
				...
				potential */
				if (ok == 1 && nleach.nleaching(ns, nf, ws, wf) == 1)
				{
				...
				if (ok == 1 && mort.mortality(epc,cs,cf,ns,nf) == 1)
				{
				...
				if (ok == 1 && checbal.check_water_balance(ws, first_balance) == 1)
				{
				...
				if (ok == 1 && checbal.check_carbon_balance(cs, first_balance) == 1)
				{
				...
				if (ok == 1 && checbal.check_nitrogen_balance(ns, first_balance) == 1)
				{
				...
				if (ok == 1 && sumfun.csummary(cf, cs, summary) == 1)
				{
				...
				if (ok == 1 && sumfun.wsummary(ws,wf,summary) == 1)
				{
				...
				have been requested */
				System.out.printf(BV_DIAG, "Number of daily outputs: %d\n", ctrl.ndayout);
				if (ok == 1 && dayout == 1)
				{
					/* fill the daily output array */
					for (outv=0 ; outv<ctrl.ndayout ; outv++)
					{
						System.out.printf(BV_DIAG, "Outv: %d, ", outv);
						System.out.printf(BV_DIAG, "DayCode: %d, ", ctrl.daycodes[outv]);
						System.out.printf(BV_DIAG, "Output: %f\n", output_map[ctrl.daycodes[outv]]);
						dayarr[outv] = (double) output_map[ctrl.daycodes[outv]];
					}
				}
				/* only write daily outputs if requested */
				if (ok == 1 && ctrl.dodaily == 1)
				{
//					/* write the daily output array to daily output file */
//					if (fwrite(dayarr, sizeof(float), ctrl.ndayout, bgcout.dayout.ptr)
//						!= (size_t)ctrl.ndayout)
//					{
//						System.out.printf(BV_ERROR, "Error writing to %s: simyear = %d, simday = %d\n",
//							bgcout.dayout.name,simyr,yday);
//						ok=0;
//					}
					
					System.out.printf(BV_DIAG, "%d\t%d\tdone daily output\n",simyr,yday);
					if(ok == 1 && bgcout.bgc_ascii == 1)
					{	
...
//						}
						...
						System.out.printf(BV_DIAG, "%d\t%d\tdone monavg output\n",simyr,yday);
...
//						}
						...
						System.out.printf(BV_DIAG, "%d\t%d\tdone annavg output\n",simyr,yday);
...
//				}
				...
				System.out.printf(BV_DIAG, "%d\t%d\tdone annual output\n",simyr,yday);
			}
...
//				steady1 = (Math.abs(t1) < SPINUP_TOLERANCE);
				...
				System.out.printf(BV_DIAG, "spinyears = %d rising = %d steady1 = %d\n",spinyears,
					rising,steady1);
				System.out.printf(BV_DIAG, "metcycle = %d tally1 = %lf tally2 = %lf pdif = %lf\n\n",
					metcycle,tally1,tally2,t1);
				if (steady1 == 1) System.out.printf(BV_DIAG, "SWITCH\n\n");
...
//				steady2 = (Math.abs(t1) < SPINUP_TOLERANCE);
				...
				{
					System.out.printf(BV_DIAG, "\nSWITCH BACK\n");

					steady1 = 0;
					rising = 1;
				}

				System.out.printf(BV_DIAG, "spinyears = %d rising = %d steady2 = %d\n",spinyears,
					rising,steady2);
				System.out.printf(BV_DIAG, "metcycle = %d tally1 = %lf tally2 = %lf pdif = %lf\n\n",
					metcycle,tally1,tally2,t1);
...
//		{
//			System.out.printf(BV_ERROR, "Error in free_phenmem() from bgc()\n");
//			ok=0;
//		}
//
//		System.out.printf(BV_DIAG, "%d\t%d\tdone free phenmem\n",simyr,yday);
