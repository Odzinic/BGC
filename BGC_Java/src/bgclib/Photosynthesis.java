...
public class Photosynthesis {
    ...
    public boolean photosynthesis(PSn psn) {
		 ...
		 * for C4 model pa (Pa) atmospheric pressure co2 (ppm) atmospheric [CO2]
		 * t (deg C) air temperature lnc (kg Nleaf/m2) leaf N concentration, per
		 * unit projected LAI flnr (kg NRub/kg Nleaf) fraction of leaf N in
		 * Rubisco ppfd (umol photons/m2/s) PAR flux density, per unit projected
		 * LAI g (umol CO2/m2/s/Pa) leaf-scale conductance to CO2, proj area
		 * basis dlmr (umol CO2/m2/s) day leaf maint resp, on projected leaf
		 * area basis
		 * 
		 * The following variables in psn struct are defined upon function
		 * return: Ci (Pa) intercellular [CO2] Ca (Pa) atmospheric [CO2] O2 (Pa)
		 * atmospheric [O2] gamma (Pa) CO2 compensation point, in the absence of
		 * maint resp. Kc (Pa) MM constant for carboxylation Ko (Pa) MM constant
		 * for oxygenation Vmax (umol CO2/m2/s) max rate of carboxylation Jmax
		 * (umol electrons/m2/s) max rate electron transport J (umol RuBP/m2/s)
		 * rate of RuBP regeneration Av (umol CO2/m2/s) carboxylation limited
		 * assimilation Aj (umol CO2/m2/s) RuBP regen limited assimilation A
		 * (umol CO2/m2/s) final assimilation rate
		 */
		 ...
		 */
		final double fnr = 7.16; /* kg Rub/kg NRub */

		/*
		 * the following enzyme kinetic constants are from: Woodrow, I.E., and
		 * J.A. Berry, 1980. Enzymatic regulation of photosynthetic CO2 fixation
		 * in C3 plants. Ann. Rev. Plant Physiol. Plant Mol. Biol., 39:533-594.
		 * Note that these values are given in the units used in the paper, and
		 * that they are converted to units appropriate to the rest of this
		 ...
		 * without the errors of big-leaf models. Plant, Cell and Env. 20:
		 * 537-557. All other parameters, including the q10's for Kc and Ko are
		 * the same as in Woodrow and Berry.
		 */
		final double Kc25 = 404.0; /* (ubar) MM const carboxylase, 25 deg C */
		final double q10Kc = 2.1; /* (DIM) Q_10 for Kc */
		final double Ko25 = 248.0; /* (mbar) MM const oxygenase, 25 deg C */
		final double q10Ko = 1.2; /* (DIM) Q_10 for Ko */
		final double act25 = 3.6; /* (umol/mgRubisco/min) Rubisco activity */
		final double q10act = 2.4; /* (DIM) Q_10 for Rubisco activity */
		final double pabs = 0.85; /* (DIM) fPAR effectively absorbed by PSII */
		...
		Ko = Ko25 * Math.pow(q10Ko, (t - 25.0) / 10.0);
		psn.Ko = Ko = Ko * 100.0; /* mbar -. Pa */
		if (t > 15.0) {
			Kc = Kc25 * Math.pow(q10Kc, (t - 25.0) / 10.0);
			act = act25 * Math.pow(q10act, (t - 25.0) / 10.0);
		} else {
			Kc = Kc25 * Math.pow(1.8 * q10Kc, (t - 15.0) / 10.0) / q10Kc;
			act = act25 * Math.pow(1.8 * q10act, (t - 15.0) / 10.0) / q10act;
		}
		...
		/*
		 * kg Nleaf kg NRub kg Rub umol umol -------- X ------- X ------- X
		...
		/*
		 * calculate Jmax = f(Vmax), reference: Wullschleger, S.D., 1993.
		 * Biochemical limitations to carbon assimilation in C3 plants - A
		 * retrospective analysis of the A/Ci curves from 109 species. Journal
		 * of Experimental Botany, 44:907-920.
		 */
		psn.Jmax = Jmax = 2.1 * Vmax;

		/*
		 * calculate J = f(Jmax, ppfd), reference: de Pury and Farquhar 1997
		 * Plant Cell and Env.
		 */
		a = 0.7;
		b = -Jmax - (psn.ppfd * pabs / ppe);
		c = Jmax * psn.ppfd * pabs / ppe;
		psn.J = J = (-b - Math.sqrt(b * b - 4.0 * a * c)) / (2.0 * a);
		 ...
		 * 
		 * Vmax (Ci - gamma) Av = ------------------- - Rd Ci + Kc (1 + O2/Ko)
		 * 
		 * 
		 * J (Ci - gamma) Aj = ------------------- - Rd 4.5 Ci + 10.5 gamma
		 */
		...
		if ((det = b * b - 4.0 * a * c) < 0.0) {
			System.out.printf(BV_ERROR, "negative root error in psn routine\n");
			ok = 0;
		}

		psn.Av = Av = (-b + Math.sqrt(det)) / (2.0 * a);
		...
		psn.Aj = Aj = (-b + Math.sqrt(det)) / (2.0 * a);
