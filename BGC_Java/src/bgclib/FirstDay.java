...
public class FirstDay {
	...
	public int firstday(final Epconst epc, final CInit cinit, Epvar epv, PhenArray phen, CState cs, NState ns) {
		...
		double max_leafc;
		double max_frootc;
		double prop_transfer;
		double transfer;
		double prop_litfall;
		double max_stemc;
		double new_stemc;
		final double GRPERC = Constant.GRPERC.getValue();
		...
		/*
		 * use then penology array information to determine, for the first day
		...
		if (woody == 1) {
			cs.gresp_transfer += (cs.livestemc_transfer + cs.deadstemc_transfer + cs.livecrootc_transfer
					+ cs.deadcrootc_transfer) * GRPERC;
		}
