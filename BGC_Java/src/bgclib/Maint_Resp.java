...
public class Maint_Resp {
	...
	public boolean maint_resp(final CState cs, final NState ns, final Epconst epc, final MetVar metv, CFlux cf, Epvar epv) {
		 ...
		 * respiration in kgC/day per kg of tissue N is: mrpern = 0.218
		 * (kgC/kgN/d)
		 * 
		...
		if (cs.leafc == 1) {
			...
			cf.leaf_day_mr = t1 * Math.pow(q10, exponent) * metv.dayl / 86400.0;
			 ...
			 */
			dlmr_area_sun = n_area_sun * mrpern * Math.pow(q10, exponent);
			dlmr_area_shade = n_area_shade * mrpern * Math.pow(q10, exponent);
			/*
			 * finally, convert from mass to molar units, and from a daily rate
			...
			cf.leaf_night_mr = t1 * Math.pow(q10, exponent) * (86400.0 - metv.dayl) / 86400.0;
		} else /* no leaves on */
		{
			cf.leaf_day_mr = 0.0;
		...
		/*
		 * ammended to consider only the specified n concentration, to avoid
		 * excessive MR with n-loading to fine roots
		 */
		if (cs.frootc == 1) {
			exponent = (metv.tsoil - 20.0) / 10.0;
			t1 = Math.pow(q10, exponent);
