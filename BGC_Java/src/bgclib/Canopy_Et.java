...
public class Canopy_Et {
	...
	public int canopy_et(final MetVar metv, final Epconst epc, Epvar epv, WFlux wf, int verbose) {
		...
		double e = 0, cwe, t = 0, trans, trans_sun, trans_shade, e_dayl, t_dayl;
		PMet pmet_in;

		double PPFD50 = Constant.PPFD50.getValue();
		...
		pmet_in = new PMet(0, 0, 0, 0, 0, 0);

		/* temperature and pressure correction factor for conductances */
		gcorr = Math.pow((metv.tday + 273.15) / 293.15, 1.75) * 101300 / metv.pa;
		...
		/*
		 * leaf stomatal conductance: first generate multipliers, then apply
		 * them to maximum stomatal conductance
		 */
		/*
		 * calculate stomatal conductance radiation multiplier: NOTE CHANGE FROM
		 * BIOME-BGC CODE *** The original Biome-BGC formulation follows the
		 * arguments in Rastetter, E.B., A.W. King, B.J. Cosby, G.M. Hornberger,
		 * R.V. O'Neill, and J.E. Hobbie, 1992. Aggregating fine-scale
		 * ecological knowledge to model coarser-scale attributes of ecosystems.
		 * Ecological Applications, 2:55-70.
		 * 
		 * gmult->max =
		 * (gsmax/(k*lai))*log((gsmax+rad)/(gsmax+(rad*exp(-k*lai))))
		 * 
		...
		/*
		 * Calculate Penman-Monteith evaporation, given the canopy conductances
		 * to evaporated water and sensible heat. Calculate the time required to
		 * evaporate all the canopy water at the daily average conditions, and
		 * subtract that time from the daylength to get the effective daylength
		 * for transpiration.
		 */
		if (canopy_w == 1) {
		...
		/*
		 * assign water fluxes, all excess not evaporated goes to soil water
		 * compartment
		 */
		wf.canopyw_evap = cwe;
		...
		/*
		 * assign leaf-level conductance to transpired water vapor, for use in
		 * calculating co2 conductance for farq_psn()
		 */
	...
	double penmon(final PMet in, int out_flag, double et) {
		 ...
		 * 
		 * For output in units of (kg/m2/s) out_flag = 0 For output in units of
		 * (W/m2) out_flag = 1
		 * 
		 * INPUTS: in->ta (deg C) air temperature in->pa (Pa) air pressure
		 * in->vpd (Pa) vapor pressure deficit in->irad (W/m2) incident radient
		 * flux density in->rv (s/m) resistance to water vapor flux in->rh (s/m)
		 * resistance to sensible heat flux
		 * 
		 * INTERNAL VARIABLES: rho (kg/m3) density of air CP (J/kg/degC)
		 * specific heat of air lhvap (J/kg) latent heat of vaporization of
		 * water s (Pa/degC) slope of saturation vapor pressure vs T curve
		 * 
		 * OUTPUT: et (kg/m2/s) water vapor mass flux density (flag=0) et (W/m2)
		...
		double CP = Constant.CP.getValue();
		double EPS = Constant.EPS.getValue();
		double SBC = Constant.SBC.getValue();
		...
		/*
		 * calculate combined resistance to convective and radiative heat
		...
		pvs1 = 610.7 * Math.exp(17.38 * t1 / (239.0 + t1));
		pvs2 = 610.7 * Math.exp(17.38 * t2 / (239.0 + t2));

		/* calculate slope of pvs vs. T curve, at ta */
		s = (pvs1 - pvs2) / (t1 - t2);

		/* calculate evaporation, in W/m^2 */
		e = ((s * in.irad) + (rho * CP * in.vpd / rhr)) / (((in.pa * CP * rv) / (lhvap * EPS * rhr)) + s);
