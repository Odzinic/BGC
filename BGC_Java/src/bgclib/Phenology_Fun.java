...
public class Phenology_Fun {
    ...
    public boolean phenology(final Epconst epc, final Phenology phen, Epvar epv, CState cs, CFlux cf, NState ns, NFlux nf) {
		...
		if (epc.evergreen == 1) {
			...
			/*
			 * litterfall happens everyday, at a rate determined each year on
			 * the annual allocation day. To prevent litterfall from driving
			 ...
			 * rate determined once each year, using the annual maximum
			 * livewoody compartment masses and the specified livewood turnover
			 * rate
			 */
			if (epc.woody == 1) {
			...
			if ((ndays = phen.remdays_transfer) == 1) {
				/*
				 * transfer rate is defined to be a linearly decreasing function
				 * that reaches zero on the last day of the transfer period
				 */
			 ...
			 * litterfall period, with a linearly ramping removal rate. assumes
			 * that the initial rate on the first day of litterfall is 0.0.
			 */
			/* AAN - yes, this is an assignment */
			if ((ndays = phen.remdays_litfall) == 1) {
				if (ndays == 1.0) {
					/*
					 * last day of litterfall, special case to gaurantee that
					 * pools go to 0.0
					 */
					leaflitfallc = cs.leafc;
					frootlitfallc = cs.frootc;
		...
		/*
		 * for woody types, find annual maximum value for live stemc and live
		 * crootc calculation of livewood turnover rates
		 */
		if (epc.woody == 1) {
