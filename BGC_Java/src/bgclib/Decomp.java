...
public class Decomp {
	...
	public boolean decomp(double tsoil, final Epconst epc, Epvar epv, final Siteconst sitec, CState cs, CFlux cf, NState ns,
		...
		double RFL1S1 = Constant.RFL1S1.getValue();
		double RFL2S2 = Constant.RFL2S2.getValue();
		double RFL4S3 = Constant.RFL4S3.getValue();
		double RFS1S2 = Constant.RFS1S2.getValue();
		double RFS2S3 = Constant.RFS2S3.getValue();
		double RFS3S4 = Constant.RFS3S4.getValue();

		double KL1_BASE = Constant.KL1_BASE.getValue();
		double KL2_BASE = Constant.KL2_BASE.getValue();
		double KL4_BASE = Constant.KL4_BASE.getValue();
		double KS1_BASE = Constant.KS1_BASE.getValue();
		double KS2_BASE = Constant.KS2_BASE.getValue();
		double KS3_BASE = Constant.KS3_BASE.getValue();
		double KS4_BASE = Constant.KS4_BASE.getValue();
		double KFRAG_BASE = Constant.KFRAG_BASE.getValue();

		/*
		 * calculate the rate constant scalar for soil temperature, assuming
		 * that the base rate constants are assigned for non-moisture limiting
		 * conditions at 25 C. The function used here is taken from Lloyd, J.,
		 * and J.A. Taylor, 1994. On the temperature dependence of soil
		 * respiration. Functional Ecology, 8:315-323. This equation is a
		 * modification of their eqn. 11, changing the base temperature from 10
		 * C to 25 C, since most of the microcosm studies used to get the base
		 * decomp rates were controlled at 25 C.
		 */
		if (tsoil < -10.0) {
			...
			tk = tsoil + 273.15;
			t_scalar = Math.exp(308.56 * ((1.0 / 71.02) - (1.0 / (tk - 227.13))));
		}
		 ...
		 * of models. Ecology, 68(5):1190-1200. and supported by data in
		...
		if (epv.psi < minpsi) {
			...
			w_scalar = Math.log(minpsi / epv.psi) / Math.log(minpsi / maxpsi);
		}
		...
		kfrag_base = KFRAG_BASE; /*
									 * physical fragmentation of coarse woody
									 * debris
									 */
