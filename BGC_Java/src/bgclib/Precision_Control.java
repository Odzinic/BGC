...
public class Precision_Control {
    ...
    public boolean precision_control(WState ws, CState cs, NState ns) {
		 ...
		 * radiation routines. Send excess to litter 1. Fine root C and N follow
		 * leaf C and N. This control is triggered at a higher value than the
		 * others because leafc is used in exp() in radtrans, and so can cause
		 * rounding error at larger values.
		 */

		double CRIT_PREC = Constant.CRIT_PREC.getValue();
		...
		/*
		 * test for litter and soil poils. Excess goes to hr sink (C) or
		 * volatilized sink (N)
		 */
