...
public class Radtrans {
	...
	public int radtrans(final CState cs, final Epconst epc, MetVar metv, Epvar epv, double albedo) {
		...
		double EPAR = Constant.EPAR.getValue();
		String BV_ERROR = String.valueOf(Constant.BV_ERROR.getValue());

		/*
		 * The following equations estimate the albedo and extinction
		 * coefficients for the shortwave and PAR spectra from the values given
		 * for the entire shortwave range (from Jones, H.G., 1992. Plants and
		 * Microclimate, 2nd Edition. Cambridge University Press. pp. 30-38.)
		 * These conversions are approximated from the information given in
		 * Jones.
		 */
		...
		if (cs.leafc > 0.0) {
			...
			epv.plaisun = 1.0 - Math.exp(-epv.proj_lai);
		...
		swabs = sw * (1.0 - Math.exp(-k_sw * proj_lai));
		swtrans = sw - swabs;
		...
		parabs = par * (1.0 - Math.exp(-k_par * proj_lai));

		/*
		 * calculate the total shortwave absorbed by the sunlit and shaded
		 * canopy fractions
		 */
		...
		/*
		 * calculate PPFD: assumes an average energy for PAR photon (EPAR,
		 * umol/J) unit conversion: W/m2 -. umol/m2/s.
		 */
